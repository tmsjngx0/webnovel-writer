...
**새로 만들지 않는다.** `humanize-korean@im-not-ai` 플러그인과 stella에 이미 설정해 둔 스킬을 쓴다. 규칙 표를 코드로 옮기는 것은 같은 판단을 두 군데에 두는 것이다.

플러그인은 세션에 상시 로드하지 않는다. 한 번만 띄운다.
...
`wn-humanize` skill이 이 호출을 감싼다. 하네스 쪽에 남길 것은 **차단 게이트가 필요해졌을 때**다. 모델 호출은 게이트가 못 된다. 매번 답이 다르기 때문이다. 줄표, 이중 피동, 대구 반복 3회 같이 정규식으로 확정되는 소수만 나중에 `wn.py lint`로 내린다. 그때까지는 안 만든다.

stella 쪽 스킬이 무엇인지 아직 확인 못 했다. `backfill` 전에 그것부터 본다.
...
| 단계 | 내용 | 막는 것 |
|---|---|---|
| 0 | stella의 기존 문체 스킬 확인 | 같은 것을 두 번 만드는 것 |
...
0단계가 먼저인 이유는 하네스에 린터를 안 만들기로 했는데 대체할 것이 무엇인지 아직 확인 못 했기 때문이다.

1단계가 그다음인 이유는 나머지가 전부 경로에 의존하기 때문이다.
