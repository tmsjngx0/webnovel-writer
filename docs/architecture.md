...
`scan`과 `concept`을 하네스로 들이기 전에, 이미 claude.ai와 ChatGPT에서 결정한 것들을 끌어와야 한다. 25화를 쓰는 동안 그쪽에 쌓였다.

일회성 수집은 **데이터 내보내기**로 한다. claude.ai 설정에서 요청하면 zip이 메일로 오고 `conversations.json`에 전부 들어 있다. 유지보수할 코드가 없다. ChatGPT도 같다.

브라우저 자동화(CDP, playwright)는 여기서 만들지 않는다. 대화를 Project로 옮기는 쓰기 작업까지 필요하고, 그것은 이 저장소의 일이 아니다. devops#534에 있다. 이 저장소는 산출물만 소비하고 그 이슈를 기다리지 않는다.
...
- **브라우저 자동화**. devops#534가 소유한다. 여기서는 데이터 내보내기로 충분하다.
